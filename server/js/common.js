...
nav_goMainIcon.onclick = function () {
  classKu.scrollGo(0, classKu.getOffsetTopByBody(body) - 140);
};
...
document.getElementsByClassName('gotoTop')[0].onclick = function () {
  classKu.scrollGo(classKu.getScrollTop(), 0,150);
};
  ...
  svgNav: function svgNav(step) {
    if (step) {
      for (var i = 0; i < navListTextactive.length; i++) {
        ...
        navListTextactive[i].textContent  = chinese[i];
        shortHover[i].style.display = 'none';
      }
      for (var i = 0; i < navListText.length; i++) {
        navListText[i].innerHTML = chinese[i];
        navListText[i].textContent  = chinese[i];
        shortHover[i].style.display = 'none';
      }
      download[0].setAttribute('href', './download/教链JLL.pdf')
      download[1].setAttribute('href', './download/教链JLL.pdf')
    } else {
      for (var i = 0; i < navListTextactive.length; i++) {
        navListTextactive[i].innerHTML = English[i];
        navListTextactive[i].textContent  = English[i];
        shortHover[i].style.display = 'block';
      }
      for (var i = 0; i < navListText.length; i++) {
        navListText[i].innerHTML = English[i];
        navListText[i].textContent  = English[i];
        shortHover[i].style.display = 'block';
      }
      download[0].setAttribute('href', './download/JLL Chain.pdf')
      download[1].setAttribute('href', './download/JLL Chain.pdf')
    }
  }
};
